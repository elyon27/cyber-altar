  ...
  const ensureProfile = useCallback(
    async (currentUsername: string, currentEmail: string, currentAltarSlug: string) => {
      ...
      if (existingProfileError) {
        throw new Error(
          `altar_profiles select failed: ${existingProfileError.message}${
            existingProfileError.details ? ` | details: ${existingProfileError.details}` : ''
          }${existingProfileError.hint ? ` | hint: ${existingProfileError.hint}` : ''}`
        );
      }

      let nextProfile = existingProfile as AltarProfile | null;
